#!/usr/bin/env bash
# Diagnose and kill orphaned backend (Python/Gunicorn) and
# frontend (Node/Vite) processes.
#
# Usage:
#   ./script/kill-zombie-processes.sh              → both envs, interactive kill
#   ./script/kill-zombie-processes.sh dev          → dev only
#   ./script/kill-zombie-processes.sh prod         → prod only
#   ./script/kill-zombie-processes.sh --check      → diagnose only, no kill prompt
...
# Fallback: hardcoded known ports
[[ -z "${BACKEND_PORTS[dev]:-}"  ]] && BACKEND_PORTS[dev]="15221"
[[ -z "${BACKEND_PORTS[prod]:-}" ]] && BACKEND_PORTS[prod]="20221"
[[ -z "${FRONTEND_PORTS[dev]:-}" ]] && FRONTEND_PORTS[dev]="15220"
[[ -z "${FRONTEND_PORTS[prod]:-}" ]] && FRONTEND_PORTS[prod]="20220"

# ---------- systemd service map (port → service unit) ----------
declare -A BACKEND_SERVICES
BACKEND_SERVICES[15221]="buyflorabella-dev.service"
BACKEND_SERVICES[20221]="buyflorabella.service"

# Return the active systemd unit that owns a PID, or empty string
unit_for_pid() {
  local pid="$1"
  local unit
...
port_pids() {
  ...
  # Primary: ss with process annotation (works when process is owned by current user)
  pids=$(ss -tlnp "sport = :$port" 2>/dev/null | grep -oP 'pid=\K[0-9]+' | sort -u)
  [[ -n "$pids" ]] && { echo "$pids"; return; }

  # Fallback: /proc/net/tcp inode → /proc/PID/fd scan.
  # ss suppresses PID info for processes it can't annotate (e.g. after a filter strips
  # the process column, or when the socket fd is held by a child reloader process).
  # This approach reads the kernel TCP table directly and matches the socket inode.
  local hex_port inode
  hex_port=$(printf '%04X' "$port")
  # Column 2 = local_address (HEX_IP:HEX_PORT), column 4 = state (0A = LISTEN), column 10 = inode
...
proc_cwd() {
  readlink -f "/proc/$1/cwd" 2>/dev/null || echo "(unknown)"
}

proc_cmd() {
  tr '\0' ' ' < "/proc/$1/cmdline" 2>/dev/null || echo "(unknown)"
}

# ---------- collect PIDs ----------
# PID_SOURCE: PID → label string
# PID_TYPE:   PID → "backend" | "frontend"
declare -A PID_SOURCE
declare -A PID_CWD
declare -A PID_CMD
declare -A PID_TYPE
FOUND_PIDS=()   # regular indexed array — no bash set-u issues for iteration
...
# Backend: processes listening on known backend ports
for env in "${ENVS[@]}"; do
...
# Backend: catch bare python app.py / gunicorn rooted in our worktrees (any port)
...
# Frontend: processes listening on known frontend ports
for env in "${ENVS[@]}"; do
...
# Frontend: catch node/vite processes rooted in our worktrees (any port)
for pid in $(ps -eo pid= 2>/dev/null); do
  [[ -d "/proc/$pid" ]] || continue
  [[ -n "${PID_SOURCE[$pid]:-}" ]] && continue
  cwd=$(proc_cwd "$pid")
...
# Keys = unit names to stop; value = 1
declare -A SYSTEMD_UNITS
declare -A PID_UNIT   # PID → unit name (empty if not systemd)
...
# Stop systemd services first (prevents immediate respawn)
for unit in "${!SYSTEMD_UNITS[@]}"; do
...
# Kill non-systemd PIDs (or any stragglers that survived the service stop)
for pid in "${FOUND_PIDS[@]}"; do
  [[ -d "/proc/$pid" ]] || continue
  unit="${PID_UNIT[$pid]:-}"
  if [[ -z "$unit" ]]; then
    ...
    # Service stop should have handled it; SIGKILL only if still alive
...
# SIGKILL any non-systemd survivors
for pid in "${FOUND_PIDS[@]}"; do
  [[ -d "/proc/$pid" ]] || continue
  [[ -n "${PID_UNIT[$pid]:-}" ]] && continue   # already handled above
...
# Hint: stopped services will NOT auto-restart; remind user how to bring them back
if [[ ${#SYSTEMD_UNITS[@]} -gt 0 ]]; then
  echo ""
